...
if ($pass != "") {
    ...
    if (password_verify($pass, $harsh_code)) {
        if ($network != "" && $phone != "" && $amount != "" && $user_id != "") {
            // finnin
        ...
        if ($status == "200" && $status != "") {
            ...
            $randms = str_pad(rand(0, pow(10, $digits)-1), $digits, '0', STR_PAD_LEFT);
            $trans = "TERA".$randms."AIRTIME".$user_id;
            // GD
            ...
            if ($update_transaction) {
                // WE ARE DONE
                $insert_transaction = mysqli_query($connection, "INSERT INTO `acct_transaction` (`user_id`, `transaction_id`, `transaction_type`, `amount`, `account_balance`, `credit`, `debit`, `transaction_date`) VALUES ('{$user_id}', '{$trans}', 'airtime_purchase', '{$amount}', '{$cal_bal}', '0.00', '{$amount}', '{$date2}')");
